...
>(
	...
	list.map(object => convertProperty(object, removedKey, replacingKey, convertFunction))

export function drop<O, K extends keyof O>(object: O, ...keys: K[]): Omit<O, K> {
...
) => list.map(obj => mergeKeyValues(obj, removedKey1, removedKey2, finalProperty, mergeFunction))
...
export const dropAllProperties = <T extends Record<string, unknown>, K extends keyof T>(
	list: T[],
	property: K
): Omit<T, K>[] =>
	list.map(obj => {
...
>(
	...
	if (typeof value === 'number') {
		...
		throw new Error(`The value of ${oldName.toString()} should be either a number or a string.`)
	}
}
...
export function convertProperty<Data extends object, NewValueType, RemovedKey extends keyof Data>(
	obj: Data,
...
export const removeUndefinedValues: <T>(obj: T) => NotUndefined<T> = <T>(obj: T) => {
	if (obj) {
